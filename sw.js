            ...
            return cache.match(event.request).then(function (response) {
                return response || fetch(event.request).then(function (response) {
                    if (!event.request.url.includes('api/session/list?subId') // check that this is not the call to the data from the server (which I don't want to cache)
                        && !event.request.url.includes('api/session?subId') // added by Rani to do the same as the one above; became necessary after the implementation of trying to resend the missed data
